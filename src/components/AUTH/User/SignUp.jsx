...
const SignUp = () => {
  ...
  // 2. React Query handles the registration server state lifecycle
  const signupMutation = useMutation({
    ...
    onError: (error) => {
      console.log("Server registration error:", error.message);
    }
  });

  // 3. Pipeline form submissions straight to the mutation execution handler
    ...
    <div className="min-h-screen flex justify-center bg-[#f5f5f6] px-4">
      <div className="mt-16 w-full max-w-md bg-white border border-gray-200 shadow-sm rounded-sm overflow-hidden">
        ...
        <div className="px-6 py-6">
          ...
          {/* 4. Dynamic Server Error Banner (e.g., "Email already exists" from backend) */}
          {signupMutation.isError && (
            <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-sm text-xs text-red-600">
              {signupMutation.error?.response?.data?.message || "Registration failed. Please check your details and try again."}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit(handleRegister)} className="space-y-4">
            ...
            {/* Submit Button Driven by React Query's Mutation Status */}
            <button
              type="submit"
              disabled={signupMutation.isPending}
              className="w-full bg-[#ff3f6c] hover:bg-[#ff1654] text-white text-sm font-semibold py-3 rounded-sm uppercase tracking-wide disabled:opacity-60 disabled:cursor-not-allowed transition-all duration-200"
            >
