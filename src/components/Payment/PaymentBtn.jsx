...
const PaymentBtn = () => {
  ...
  const {
    ...
    totalMRP,
    discount,
    items = [], // <- EXPECT cart items here: [{ productId, name, qty, price }, ...]
  } = location.state || {};
    ...
    try {
      const response = await fetch(
        "https://e-commerce-project-76em.onrender.com/payment/create-order",
        {
          ...
          body: JSON.stringify({
            order_amount: amount,
            items, // send cart items so backend can save them
      ...
      // Optional: store orderId so you can reference it client-side if needed
      if (data.orderId) {
        try {
    ...
    <div style={{ textAlign: "center", marginTop: "50px" }}>
      ...
      >
        ...
        <div style={{ textAlign: "left", marginBottom: "10px" }}>
          ...
          <p>
            <strong>Total MRP:</strong> ₹{totalMRP || 0}
          </p>
          <p>
